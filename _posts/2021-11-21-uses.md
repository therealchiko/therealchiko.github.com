...
My workspace has certainly evolved so much over the years, from the time when I used a broken laptop we called "a refrigerator" (wanna guess what would happen if the power cord moved?) to now with my motorized desk that beeps when it's time for me to stand.  My decision process when picking equipment is pretty straightforward, I identify a weakness, deep dive into understanding about the component, make a pick and quickly return it if it's not up to par. In essence, if it's listed here - it works very well __for me__, YMMV. Without further ado, here is the list of my hardware:
<!--more-->
